...
iris.constant("userStatsURL", "api/stats/{projectID}/userstatistics.json");
...
//				$log.debug(callbackError)
            ...
            // get the annotations for a given project, images, terms and users
            fetchUserStatistics : function(projectID, imageIDs, termIDs, userIDs, callbackSuccess,
                                                    callbackError, offset, max) {
                $log.debug("Getting user statistics: " + projectID + " - " + imageIDs + " - " + termIDs);

                // modify the parameters
                var url = cytomineService.addKeys(userStatsURL).replace("{projectID}", projectID);
