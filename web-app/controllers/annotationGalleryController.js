...
iris.controller("annotationGalleryCtrl", [
	...
	"navService", "annotationService", "sessionService", "sharedService", "settingsService", "$routeParams",
	function($rootScope, $scope, $http, $filter, $log,
		...
		$scope.checkAccess = function (projectID, userID) {
			$log.debug("Checking access to annotation gallery...");
				...
				function (data) {
					if (data === undefined || data.length === 0){
						$scope.annotation.error = {
							forbidden: {
								status : 403,
								message : "The project is not available on this IRIS host."
							}
						};
						$scope.annotation.checkingAccess = false;
					}
					else if (data !== undefined && data.length !== 0
						&& data[0].projectSettings.enabled === false){
			...
			if (targetTermID == -99){
				...
				annotationService.deleteAnnotationTerm(item.cmProjectID, item.cmImageID, item.cmID, targetTermID,
					function(data){
						...
						sharedService.addAlert("Term '" + termName + "' has been removed from [" + item.cmID + "].");
				...
				annotationService.setAnnotationTerm(item.cmProjectID, item.cmImageID, item.cmID, targetTermID,
					function(data){
						...
						sharedService.addAlert("Term '" + termName + "' has been assigned to [" + item.cmID + "].");
					}, function(data, status, config, header){
		...
		$scope.fetchAnnotations = function(termIDs, imageIDs, offset, loadPage){
			...
			// perform the query
			annotationService.fetchUserAnnotationsByTerm($scope.projectID,
...
//    		    	$log.debug("Resolving group for term '" + $rootScope.termList[termID] + "'")
							...
							for (var i = 0; i < group.pageItems; i++){
								...
								annotation.checked = selectedAnns.indexOf(annotation.cmID + "") != -1;
							}
...
//    	$log.debug(event.name + "::" + action + ": "  + imageList.ids + ".");
		...
		$scope.labelAnnotation = function(ann){
			...
			sessionService.openImage(ann.cmProjectID, ann.cmImageID, function(data){
				$log.debug("Successfully updated image " + ann.cmImageID + " in user session");
				delete $scope.error;
		...
		$scope.selectAllAnnotations = function(termID){
			$log.debug("Checking all annotations on the current page of group " +  $rootScope.termList[termID]);
			var allAnnotations = getGroup($scope.annotation.groups, termID).annotations;
			for (var idx = 0; idx < allAnnotations.length; idx++){
				var ann = allAnnotations[idx];
				ann.checked = true;
		...
		$scope.unSelectAllAnnotations = function(termID){
			$log.debug("UNChecking all annotations on the current page of group " +  $rootScope.termList[termID]);
			try {
				var allAnnotations = getGroup($scope.annotation.groups, termID).annotations;
				for (var idx = 0; idx < allAnnotations.length; idx++){
					var ann = allAnnotations[idx];
				...
				$log.debug("Group " + $rootScope.termList[termID] + " is not visible.")
			}
		...
		$scope.assignTermToSelectedItems = function(){
			...
			for (var idx = 0; idx < nSelections; idx++){
				...
				if (targetTermID == -99){
					...
					$log.debug("Assigning term " + termName + " to annotation " + item.cmID);
