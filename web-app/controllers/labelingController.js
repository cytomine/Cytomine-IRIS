...
iris
		.controller(
				"labelingCtrl",
				function($scope, $http, $filter, $location, $routeParams, $log,
						hotkeys, helpService, annotationService,
						projectService, sessionService, sharedService,
						imageService, cytomineService) {
					...
					annotationService.fetchUserAnnotations($scope.projectID,
							$scope.imageID, function(data) {
								$log.debug("retrieved " + data.length
										+ " annotations");
								...
								sharedService.addAlert(
										"Cannot get user annotations! Status "
												+ status + ".", "danger");
							});
					...
					$scope
							.$watch(
									'currentPage',
									function() {
										// $log.debug("Setting current page to "
										// + $scope.currentPage)
										// switch through the array of
										// annotations
										$scope.annotation = $scope.labeling.annotations[$scope.currentPage - 1];

										try {
											$log.debug($scope.annotation);
											sessionService.touchAnnotation(
													$scope.projectID,
													$scope.imageID,
...
iris.controller("termCtrl",
		function($scope, $log, $filter, $routeParams, $document, sharedService,
				sessionService, projectService, ngTableParams) {
			...
			$scope.assign = function(term) {
				...
				sharedService.addAlert("Term " + term.name
						+ " has been assigned.");
			}
			...
			$scope.reset = function() {
				// TODO unselect all terms (e.g. if term is successfully
				// removed)
